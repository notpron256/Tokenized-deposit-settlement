...
Files: `backend/src/jobs/sanctionsSync.ts` (fetch + parse + on-chain sync-write), `backend/scripts/sanctions-sync.ts` (CLI: `--dry-run` for fetch/parse/report only, or a full run that also writes on-chain), `backend/src/routes/sanctions.ts` (`POST /sanctions/sync`, the UI's "Sync Now" action), frontend Compliance page addition (a "Sync Now" button + result summary above the Sanctions registry view).

Fetches the live SDN list from OFAC's own Sanctions List Service (`sanctionslistservice.ofac.treas.gov` — the legacy `treasury.gov` paths now 302-redirect here), extracts every `Digital Currency Address - SOL` entry, and independently validates each is a genuine Solana public key before writing it on-chain — never trusting OFAC's own currency-type label alone. Every entry written by the real OFAC fetch is tagged `source: OfacSdn`; the synthetic test entry (seeded once, separately) stays tagged `source: SyntheticTest` and is never overwritten or relabeled by a sync run — the full-replace write always reads the current registry first and carries its `SyntheticTest` entries forward.

**Real run against devnet, 09/04/2026 SDN publication:** 19,329 total SDN entries; 1,007 digital currency addresses across 20 currency tags; 4 tagged `Digital Currency Address - SOL`, all 4 independently confirmed as valid Solana public keys (3 distinct sanctioned parties). Dry run reviewed and confirmed before any on-chain write; a second, idempotent re-sync via the real "Sync Now" button produced the identical 4 real + 1 synthetic = 5 total. Verified via a fresh on-chain read afterward (both API and live UI) showing the correct `REAL (OFAC SDN)` / `SYNTHETIC (TEST)` badge on each entry (see spec-001.md, Areas of concern, for the full account and the sparsity finding's implications for sync cadence — Open questions).

**Done test:** click "Sync Now" — UI shows "fetched N SDN entries, M Solana-format addresses found, registry updated," and the registry-entries view (Phase 6) shows the newly synced entries badged "REAL (OFAC SDN)" while the synthetic test address from Phase 0.5/2 remains badged "SYNTHETIC (TEST)." Verified live via the actual browser UI, not just the API.

### Phase 8 — Redeem/burn flow ✅
Files: `programs/redemption-gateway` (the real gateway program, replacing the Phase 0.5 spike — same verified two-Signer co-sign mechanism, upgraded from raw `Burn` to `BurnChecked`), `backend/src/flows/redeemFlow.ts` (sanctions re-check → co-signed burn → ledger update), `backend/src/routes/redeem.ts`, frontend Redeem page.

A fresh program ID was generated for the real program (`A4JWxQpSW19yZ27bFR9Gfxz14SxVDhExQoXvixt3zVzN`) rather than reusing the spike's throwaway one, and deployed local-first (this phase — unlike Phase 6/7 — involves genuine new on-chain program logic, not just calling an already-verified instruction with new data) before devnet.

**Ledger semantics — resolved: redemption follows the clawback pattern, not the mint pattern.** An earlier draft of this build read spec-001.md step 8's "credited" as an instruction to *decrease* both `cash_balance_cents` and `tokenized_cents` together, mirroring Fund/mint's increase in reverse — corrected after review. intent-001.md frames redemption as converting the token back to cash the client holds *at the bank*, not a withdrawal out of the bank entirely; `cash_balance_cents` already represents the client's TOTAL deposit liability regardless of tokenized status (exactly what Fund/mint's "both increase together" means — a deposit increases total liability and tokenizes all of it in one action). Redemption is the inverse of that one action, un-tokenizing rather than extinguishing the liability, so **only `tokenized_cents` decreases; `cash_balance_cents` is left untouched**, exactly like a clawback. The earlier "both decrease" interpretation was implemented, verified, then found wrong and corrected — including retroactively fixing the `cash_balance_cents` of every redemption already executed under it (see spec-001.md's Areas of concern for the corrected rows).

**Real verification, both networks:**
- **Local:** a normal client (Acme Corp Treasury) redeemed $500.00 — on-chain balance and `tokenized_cents` decreased by exactly that amount and matched afterward; `cash_balance_cents` correctly untouched. Sanctioned Test Corp's redemption attempt was refused (`refused_sanctioned`, no `tx_signature`) — independently confirmed via a direct on-chain balance read before and after ($620,149.82, unchanged) that literally nothing was submitted, not just that the API returned an error.
- Additionally verified the two-signer co-sign gate itself is real on-chain enforcement, not just backend discipline: a deliberately under-signed `redeem` instruction (client-only, no compliance signer) against the newly-deployed program was rejected with "Signature verification failed" — reproducing the exact result the Phase 0.5 spike found, now confirmed against the real, separately-deployed program rather than assumed to carry over.
- **Devnet:** program deployed fresh; the same two scenarios re-verified through the real running backend and, for the successful case, through the actual browser UI end-to-end (Devnet Alpha Holdings redeemed $25.00, balances updated live) — the sanctioned rejection was also confirmed live in the UI with the SYNTHETIC/TEST badge shown, and independently against a direct on-chain balance read before/after ($619,200.00, unchanged).

**Done test:** click "Redeem $X" for a normal client — token balance decreases, ledger cash balance moves, tokenized flag released. Then attempt redemption for the sanctioned test client — refused before any burn is attempted, with the sanctions badge shown in the rejection reason. Verified exactly this way, live, on both networks.

### Phase 9 — Reconciliation ✅ (background 5-minute interval deliberately deferred)
Files: `backend/src/jobs/reconciliation.ts` (aggregate + per-client checks), `backend/src/routes/reconciliation.ts` (`POST /reconciliation/run`, `GET /reconciliation/breaks`), `backend/scripts/inject-break.ts`, frontend Reconciliation page. The background 5-minute interval named in this phase's original file list is deliberately deferred, not an oversight — the same "no continuous supervision infrastructure for this POC" precedent already established for the Phase 6 indexer (spec-001.md, Areas of concern): an on-demand "Run Reconciliation" trigger is sufficient for demo purposes, and adding a real always-on scheduled job (with the process-supervision, crash-recovery, and alerting concerns that implies) would be exactly the kind of production-hardening this POC's scope has consistently deferred elsewhere rather than half-built.

Both invariants compare against `tokenized_cents` specifically, never `cash_balance_cents` (consistent with clawback/redemption both only ever moving the tokenized side). On-chain balances are read live and directly from the chain (`getAccount`/`getMint`), never from anything this backend's own flows wrote to Postgres. A detected per-client break is further classified: if it exactly matches the sum of that client's `confirmed`-but-not-`settled` events (a real `tx_signature`, so the chain action genuinely happened — plan-001.md decision #5's "a stuck pending_chain row is exactly what reconciliation is meant to surface"), it's labeled `in_flight`, not a data-integrity error.

**Two real bugs found and fixed by actually running this against real data, not just typechecking it:**
- `runReconciliation`'s client query originally selected `tokenized_cents` directly from `clients` — a column that lives on `ledger_balances`, not `clients`. Failed immediately on first real run; fixed with a join.
- The aggregate check's first real run (after the join fix) found a genuine-looking $7,210.00 discrepancy on local — which turned out to be exactly the sum of two real local clawbacks. The bank recovery ATA (Phase 6.5's clawback destination) is a real, legitimate, non-burned token holder outside the client set; the aggregate's "expected" side now includes its live balance too, or every clawback that ever happened would permanently look like an unexplained break. Confirmed by re-running: local dropped straight back to `allClear: true` (with two real clawbacks in its history), and devnet's first-ever run was clean on the first try (three real clawbacks in its history, `allClear: true` immediately).
- Separately (not a Phase 9 bug, but found while establishing the local baseline): 6 local `deposit_events` rows were stuck at `status = 'confirmed'` from before the settlement-finality gating fix existed, when `confirmed` was still the terminal status and ledger updates happened there directly — never migrated forward to `settled` when that fix landed. Their ledger effects were already correctly applied (confirmed via the clean baseline itself showing no break for any of those clients); only the stale status label was corrected, not any balance.

**Real, deliberately-broken runs — both networks:**
- **Local:** clean baseline (`allClear: true`) → `inject-break.ts "Sunrise Capital" 500000` → re-run correctly surfaced both an aggregate break and a per-client break (client ID, expected, actual, delta all present), both `unexplained` (correct — genuine corruption, not a timing artifact) → reverted.
- **Local, stuck-event scenario:** a real on-chain transfer ($75.00, Beta LLC Operating → Timing Test Corp) submitted directly, deliberately skipping the ledger update transferFlow.ts would normally apply at settlement — simulating a crash between `confirmed` and `settled` with a real signature and a real, on-chain-matching amount. Reconciliation correctly flagged *both* sides (sender and recipient) as `in_flight`, each with a note naming the specific matching event, and — notably — the *aggregate* check stayed clean throughout, since a misallocation between two existing clients doesn't change total supply: a live demonstration of exactly why the per-client check is the authoritative one, not the aggregate. Ledger update completed properly afterward, confirmed back to clean.
- **Devnet:** identical sequence re-run against real devnet data (4 real clients, real clawback/redemption history already in place) — clean baseline, a real injected break correctly caught (Devnet Alpha Holdings, $1,000.00), a real stuck transfer ($3.00, Devnet Beta Treasury → Devnet Alpha Holdings) correctly classified `in_flight` on both sides, both reverted/completed and confirmed back to clean. The full sequence (a fresh clean run, then the historical break records) was also verified live through the actual browser UI, not just the API.
