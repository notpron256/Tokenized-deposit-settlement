...
File: `backend/src/indexer.ts` — subscribes to the Transfer Hook program's logs across every client ATA and durably logs **all transfer activity it independently observes on-chain**: signature, both parties, amount, the memo's reference/identity-hash fields, and any flag or rejection reason visible in the logs (large-transaction flags, blocked-transfer reasons, and successful transfers alike) — not only `LargeTransactionFlag` events as originally scoped. Writes to a new indexer-owned table (e.g. `indexed_transfers`), kept separate from `transfer_events`/`deposit_events` (backend/src/flows/*.ts) since those are written by the backend's own flows and would prove nothing about correctness if the indexer just mirrored them. Frontend: Compliance page (flag list, plus a registry-entries view that renders each sanctions entry's `source` tag as a "REAL (OFAC SDN)" or "SYNTHETIC (TEST)" badge — the badge comes from the on-chain `source` field, not a hand-maintained UI list).
